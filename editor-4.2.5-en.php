...
Min_Driver($g);if(!is_object($g)||($id=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?$g:(is_string($id)?$id:'Invalid credentials.')));if($Ca&&$_POST["token"])$_POST["token"]=$uf;$m='';if($_POST){if(!verify_token()){$Kc="max_input_vars";$qd=ini_get($Kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$qd||$X<$qd)){$Kc=$y;$qd=$X;}}}$m=(!$_POST["token"]&&$qd?sprintf('Maximum number of allowed fields exceeded. Please increase %s.',"'$Kc'"):'Invalid CSRF token. Send the form again.'.' '.'If you did not send this request from Adminer then close this page.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('Too big POST data. Reduce the data or increase the %s configuration directive.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
